...
def pull_weather_data(**kwargs):
	...
	url = "https://api.openweathermap.org/data/3.0/onecall?lat={}&lon={}&appid={}&dt={}"
	payload={}
	headers = {}
	...
	response = requests.request(
		"GET", url.format(city_coordinates[0], city_coordinates[1], api_key, timestamp), headers=headers, data=payload
	)
...
with DAG(dag_id="weather", schedule_interval="@daily", start_date=days_ago(2)) as dag:
	...
	for counter, city_name in enumerate(values):
		python_task = PythonOperator(
			...
			op_kwargs={'city_name': city_name, 'timestamp': ts},
		)
